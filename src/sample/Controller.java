            ...
            Platform.runLater(() -> {
                ...
                while (!flag) {
                    ...
                    if(newAnalizer.state.operation.equals("Shift")){
                        ...
                        int finalCount = GridPane.getColumnConstraints().size() + 1;
                        /*
                        Добавляет колонку если поступающих
                        символов больше чем колонок
                        */
                        if (GridPane.getColumnConstraints().size() < finalCount) {
...
//                                }
                                ...
                                label.setScaleX(2);
                                label.setScaleY(2);
                                if(label.getText().equals("") && GridPane.getColumnConstraints().size() > 1)
                                    label.setText("—\t" + newAnalizer.simvoli.substring(matcher.start(), matcher.end()));
...
//                        fxViewPanel.getViewer().getGraphicGraph().getNode(Integer.toString(id)).setAttribute("ui.label", newAnalizer.simvoli );
                        ...
                        символов больше чем колонок
                        */
                        if (GridPane.getColumnConstraints().size() < (finalCount -1)) {
                            ColumnConstraints column = new ColumnConstraints();
                            column.setPercentWidth(50);
                            column.setHalignment(HPos.CENTER);
                            column.setHgrow(Priority.SOMETIMES);
                        ...
                        */
                        ...
                        for(int k = 0; k < GridPane.getColumnConstraints().size(); k++){
                            for (int i = GridPane.getRowConstraints().size() - 1; i > 0; i--) {
                        ...
                        while (matcher.find()) {
                            if(i == (finalCount - 1) && newAnalizer.reduceCount != 0) {
                                Label label = (Label) getNodeByRowColumnIndex(0, GridPane.getColumnConstraints().size() - 1 - (newAnalizer.reduceCount - 1), GridPane);
